...
class Box : public Hitable {
  public:
    Box() {}
    Box(const glm::vec3 &cen, const glm::vec3 r, std::vector<Triangle> m) : center(cen), radius(r), meshes(m){};
...
bool Box::hit(const Ray &r, float t_min, float t_max, HitRecord &rec) const
{
    ...
    if (temp1 <= temp2 && temp1 < t_max && temp > t_min) {
        for(Triangle currMesh : meshes) {
            ...
            if (currMesh.hit(r, t_min, t_max, rec)) {
                return true;
            }
        }
        //return true;
